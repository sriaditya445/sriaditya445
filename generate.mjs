...
const USERNAME = process.env.GH_USERNAME || "sriaditya445";
...
async function fetchWeeks() {
  if (!TOKEN) {
    console.warn("No GH_TOKEN provided. Generating fallback contribution grid.");
  ...
  if (!res.ok) {
    console.warn(`GitHub API error ${res.status}: using fallback grid.`);
    return generateFallbackWeeks();
  }
  const json = await res.json();
  if (json.errors || !json.data?.user?.contributionsCollection?.contributionCalendar?.weeks) {
    console.warn("GitHub API response invalid, using fallback grid.");
    return generateFallbackWeeks();
  }
...
function generateFallbackWeeks() {
  ...
  return Array.from({ length: COLS }, () => ({
    contributionDays: Array.from({ length: ROWS }, () => {
      const r = Math.random();
      const level = r > 0.7 ? (r > 0.9 ? 4 : r > 0.8 ? 3 : 2) : (r > 0.4 ? 1 : 0);
      return {
